...
install.packages(c("sf", "terra", "raster", "ncdf4"),
                 type = "source")
...
all_files <- list.files(nc_save_path,"\\.nc$", full.names = T)
file_info <- str_match(basename(all_files), "^ct5km_(.{3,4})-(.{3,4})_v3.1_(.{4})\\.nc$")
parquet_save_path <- "/Users/uqkbairo/MODRRAP/storage1tb/data/annual/"


r <- rast(all_files[1], subds = "degree_heating_week")
r

r <- brick(all_files[1], xmn=0, xmx=360, ymn=-90, ymx=90)
r2 <- rotate(r) # rotate these values from [0°,360°] longitude to regular [-180°,180°] longitude

brick(all_files[1])
library(ncdf4)
nc <- nc_open(all_files[1])
print(nc)

nc_file = paste0(nc_save_path, all_files[1])

read_nc_to_df <- function(nc_file) {
    nc <- nc_open(nc_file)
    on.exit(nc_close(nc))
    ...
    expand.grid(
        ...
        mutate(
            value = as.vector(ncvar_get(nc, "your_variable_name"))
        )
}

read_nc_to_df(nc_file)
